...
;(function (factory) {
    ...
    editormd.defaults = {
        ...
        parseAjaxResponse: function (res) {
            if (res.status) {
                return res.data[0];
            } else {
                return null;
    ...
    editormd.prototype = editormd.fn = {
        ...
        loadQueues: function () {
            ...
            if (settings.imageUpload) {
                if (settings.resizeImageBeforeUpload) {
                    editormd.loadScript(loadPath + "localResizeIMG/lrz.bundle")
                }
                if (!!settings.sts) {
                    // 开启oss服务
                    editormd.loadScript(loadPath + "aliyun-oss-sdk.min")
                }
                if (settings.autoUploadPasteImage) {
                    this.editor.on('paste', function (event) {
                        _this.uploadPaste(event, settings);
                    });
                }
            }
            return this
         ...
         * @private
         */
        fileNameGenerator(originalFilename) {
            var date = new Date();
            var yyyy = date.getFullYear();
            var MM = (date.getMonth() + 1 < 10 ? '0' + (date.getMonth() + 1) : date.getMonth() + 1);
            var dd = date.getDate();
            var hh = (date.getHours() + 1 < 10 ? '0' + (date.getHours() + 1) : date.getHours() + 1);
            var mm = (date.getMinutes() + 1 < 10 ? '0' + (date.getMinutes() + 1) : date.getMinutes() + 1);
            var ss = (date.getSeconds() + 1 < 10 ? '0' + (date.getSeconds() + 1) : date.getSeconds() + 1);
            var name_1 = yyyy + MM + dd + hh + mm + ss;
            var name_2 = new Date().getTime();
            var suffix = originalFilename.substring(originalFilename.lastIndexOf("."), originalFilename.length);
            return name_1 + name_2 + suffix;            //新文件
        },

        _cachedStsToken: null,
        _stsTokenRequest(callback) {
            var settings = this.settings;
            var _this = this;
            $.ajax({
                url: settings.sts,
                method: 'post',
                data: settings.stsRequestParams,
                dataType: 'json',
                beforeSend: function (xhr) {
                    var headers = settings.stsRequestHeaders;
                    var keys = Object.keys(settings.stsRequestHeaders);
                ...
                success: (res) => {
                    if (res.status) {
                        var token = res.data;
                        var times = new Date(token.expiration).valueOf();
                        token.expirationTimes = +times;
        ...
        stsToken(callback) {
            if (!!this._cachedStsToken) {
                // 有缓存 （提前5分钟更新）
                if (this._cachedStsToken.expirationTimes <= new Date().valueOf() - 300000) {
                    // Token已过期
                    this._stsTokenRequest(callback);
                } else {
                    callback(this._cachedStsToken);
                }
            } else {
                // 无缓存Token
                this._stsTokenRequest(callback);
         ...
         * OSS 上传
         */
        ossUpload(file, type, callback) {
            var originName = file.name;
            var key;
            if (type === 'image') {
                if (!originName || originName.indexOf('.') === -1) {
                    originName = 'image.jpeg';
                }
         ...
         * 普通上传
         */
        ajaxUpload(file, callback) {
            var settings = this.settings;
            var formData = new FormData();
            formData.append(settings.ajaxUploadFileKey, file);
            var params = settings.ajaxUploadParams;
            ...
            $.ajax({
                ...
                dataType: 'json',
                beforeSend: function (xhr) {
                    var headers = settings.ajaxUploadHeaders;
                    var keys = Object.keys(settings.ajaxUploadHeaders);
                    for (var i = 0; i < keys.length; i++) {
                        xhr.setRequestHeader(keys[i], headers[keys[i]]);
                    }
                },//这里设置header
